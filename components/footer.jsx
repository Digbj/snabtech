import React from "react";
const Footer=()=>{
    return(
        <div className="flex flex-col mt-24">
            <div className="flex text-left justify-between list">
            <div className="">
                <p>Page d'accueil</p>
                <ul>
                    <li>Missions de PANDA</li>
                    <li>Fonctionnalités</li>
                    <li>Découvrez les experts</li>
                    <li>Découvrez les organisations en Afrique</li>
                    <li>Offres d'emploi</li>
                    <li>Evénements à venir</li>
                    <li>Témoignages</li>
                    <li>Témoignages</li>
                </ul>
            </div>
            <div>
                <p>pour les EXPERTS</p>
                <ul>
                    <li>Découvrez les organisations en Afrique</li>
                    <li>Offres d'emploi</li>
                    <li>Travel Map</li>
                    <li>Evénements à venir</li>
                </ul>
            </div>
            <div>
                <p>pour les organisations en Afrique</p>
                <ul>
                    <li>Découvrez les experts</li>
                    <li>Travel Map</li>
                    <li>Evénements à venir</li>
                </ul>
            </div>
            <div>
                <p>Contact </p>
                <ul>
                    <li>Nous contacter</li>
                </ul>
            </div>
            </div>
           
            <div className="flex items-center justify-between mt-5">
                <div>PANDA</div>
                <p>© 2022 Tous droits réservés</p>
                <div>
                <svg xmlns="http://www.w3.org/2000/svg" width="116" height="16" viewBox="0 0 116 16" fill="none">
  <path fill-rule="evenodd" clip-rule="evenodd" d="M109.312 4.96133L109.34 5.43711L108.871 5.37885C107.161 5.15552 105.668 4.39816 104.4 3.12617L103.78 2.49504L103.62 2.96111C103.282 4.00006 103.498 5.09727 104.202 5.83521C104.578 6.24302 104.494 6.30128 103.845 6.05854C103.62 5.98086 103.423 5.9226 103.404 5.95173C103.338 6.0197 103.564 6.90329 103.742 7.25285C103.986 7.73834 104.484 8.21412 105.029 8.4957L105.489 8.71903L104.944 8.72874C104.418 8.72874 104.4 8.73845 104.456 8.94235C104.644 9.57349 105.386 10.2435 106.212 10.5348L106.795 10.7387L106.288 11.0494C105.536 11.496 104.653 11.7485 103.77 11.7679C103.348 11.7776 103 11.8165 103 11.8456C103 11.9427 104.146 12.4864 104.813 12.7001C106.814 13.3312 109.19 13.0593 110.975 11.9815C112.243 11.2145 113.511 9.69001 114.103 8.21412C114.422 7.42762 114.742 5.99057 114.742 5.30117C114.742 4.85452 114.77 4.79626 115.296 4.26222C115.606 3.95151 115.897 3.61166 115.953 3.51457C116.047 3.33008 116.038 3.33008 115.559 3.49515C114.76 3.78644 114.648 3.7476 115.042 3.31066C115.333 2.99995 115.681 2.43678 115.681 2.27171C115.681 2.24258 115.54 2.29113 115.38 2.37852C115.211 2.47562 114.835 2.62126 114.554 2.70865L114.046 2.87372L113.586 2.55329C113.333 2.37852 112.976 2.18432 112.788 2.12606C112.309 1.99012 111.576 2.00954 111.144 2.1649C109.97 2.60184 109.228 3.72818 109.312 4.96133Z" fill="#1F1F1F"/>
  <path fill-rule="evenodd" clip-rule="evenodd" d="M53 14.0352H50V5.03516H53V14.0352Z" fill="#1F1F1F"/>
  <path fill-rule="evenodd" clip-rule="evenodd" d="M51.4911 3.03516H51.474C50.5788 3.03516 50 2.36818 50 1.53464C50 0.683462 50.5964 0.0351562 51.5087 0.0351562C52.4209 0.0351562 52.9827 0.683462 53 1.53464C53 2.36818 52.4209 3.03516 51.4911 3.03516Z" fill="#1F1F1F"/>
  <path fill-rule="evenodd" clip-rule="evenodd" d="M63.9999 14.0349H61.0519V9.33051C61.0519 8.14886 60.6253 7.34254 59.5581 7.34254C58.7437 7.34254 58.2586 7.88612 58.0455 8.41114C57.9675 8.5993 57.9484 8.86146 57.9484 9.12426V14.0352H55C55 14.0352 55.0389 6.06699 55 5.24187H57.9484V6.48737C58.3397 5.88864 59.0405 5.03516 60.6057 5.03516C62.5456 5.03516 64 6.29221 64 8.99312L63.9999 14.0349Z" fill="#1F1F1F"/>
  <path fill-rule="evenodd" clip-rule="evenodd" d="M1.55073 15.0352V7.53457H0V4.94973H1.55073V3.3978C1.55073 1.28908 2.42638 0.0351562 4.91418 0.0351562H6.98534V2.62029H5.69072C4.72228 2.62029 4.65821 2.98153 4.65821 3.65569L4.65469 4.94943H7L6.72556 7.53428H4.65469V15.0352H1.55073Z" fill="#1F1F1F"/>
</svg>
                </div>
            </div>
        </div>
    )
}
export default Footer;